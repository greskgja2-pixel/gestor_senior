...
/** Atualiza campos básicos de um anúncio pela Shopee Open Platform.
 * Na primeira fase do Gestor Sênior a rota publica somente item_name e description.
 */
export async function updateItem({ shopId, accessToken, itemId, fields = {} }) {
    const id = Number(itemId);
    if (!Number.isSafeInteger(id) || id <= 0) throw new Error("itemId inválido para update_item.");
    if (!fields || typeof fields !== "object" || Array.isArray(fields) || !Object.keys(fields).length) {
          throw new Error("Nenhum campo informado para update_item.");
    }
    return shopPost("/api/v2/product/update_item", {
          shopId,
          accessToken,
          body: { item_id: id, ...fields },
    });
}
