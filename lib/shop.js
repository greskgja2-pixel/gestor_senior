...
// Desconecta a loja NO PAINEL sem apagar a credencial: marca paused=true. getActiveShop()
// passa a devolver null (painel mostra "desconectada"), mas o access_token/refresh_token
// continuam guardados. Isso evita que o lojista precise refazer o login completo da Shopee
// (com a tela de escolher a validade da autorização e, às vezes, confirmação por e-mail)
// só para reconectar — "Entrar com a Shopee" nesse caso apenas desmarca paused (resumeShop).
export async function logoutShop() {
  const db = supabaseAdmin();
  const { error } = await db.from("shop_credentials").update({ paused: true }).not("id", "is", null);
  if (error) throw new Error(`Erro desconectando a loja: ${error.message}`);
}

// Reconecta a loja pausada sem passar pela autorização da Shopee de novo: só desmarca
// paused. Se o access_token estiver vencido, a próxima chamada a getActiveShop() renova
// sozinho com o refresh_token (que ainda é válido por até ~30 dias desde o último uso).
export async function resumeShop() {
  const db = supabaseAdmin();
  const { error } = await db.from("shop_credentials").update({ paused: false }).not("id", "is", null);
  if (error) throw new Error(`Erro reconectando a loja: ${error.message}`);
}

// Estado de conexão pra UI: diferencia "nunca autorizado" de "pausado" (credencial guardada,
// só escondida do painel) de "conectado". Usado pelo /api/shopee/connection.
export async function getConnectionState() {
  ...
  const { data, error } = await db
    ...
    .maybeSingle();
  if (error) throw new Error(`Erro lendo shop_credentials: ${error.message}`);
  if (!data) return { connected: false, paused: false, shopId: null };
  if (data.paused) return { connected: false, paused: true, shopId: String(data.shop_id) };
  const shop = await getActiveShop();
  return { connected: Boolean(shop), paused: false, shopId: shop ? String(shop.shop_id) : null };
}
