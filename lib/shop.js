...
// Desconecta a loja: apaga a credencial guardada. Como é single-shop, apaga todas as
// linhas de shop_credentials — depois disso getActiveShop() volta a retornar null e o
// lojista precisa autorizar de novo pelo botão "Entrar com a Shopee".
export async function logoutShop() {
  const db = supabaseAdmin();
  const { error } = await db.from("shop_credentials").delete().not("id", "is", null);
