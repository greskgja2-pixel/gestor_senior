...
async function syncFromShopee(shop) {
    ...
    const pageSize = 50;
    const allItemIds = [];
    const MAX_PAGES = 60; // ate 3000 itens - trava um loop infinito por resposta inesperada da Shopee
...
/** Devolve os produtos da loja: do cache por padrao, ou buscando na Shopee se `forceRefresh` ou cache vazio. */
...
/** Conta ao vivo (sem cache) quantos itens a loja tem com status BANNED e UNLIST na Shopee.
 *  Usa o total_count devolvido por get_item_list, sem baixar os itens em si.
 *  Se a Shopee falhar pra algum dos dois status, aquele contador volta null (nunca inventa numero). */
export async function getProductStatusCounts(shop) {
  const statuses = ["BANNED", "UNLIST"];
  const counts = {};
  for (const itemStatus of statuses) {
    try {
      const page = await getItemList({ shopId: shop.shop_id, accessToken: shop.access_token, offset: 0, pageSize: 1, itemStatus });
      const total = page.response?.total_count;
      counts[itemStatus] = Number.isFinite(total) ? total : null;
    } catch {
      counts[itemStatus] = null;
    }
  }
  return { banned: counts.BANNED, unlist: counts.UNLIST };
}
