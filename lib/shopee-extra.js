...
export function formatShopeeDate(date){const d=new Date(date);return`${String(d.getDate()).padStart(2,"0")}-${String(d.getMonth()+1).padStart(2,"0")}-${d.getFullYear()}`;}
export function dateRange(days=7){const end=new Date(),start=new Date();start.setDate(end.getDate()-Math.max(0,days-1));return{startDate:formatShopeeDate(start),endDate:formatShopeeDate(end)};}
export async function getAdsDaily(shop,days=7){const{startDate,endDate}=dateRange(days);return shopGetExtra("/api/v2/ads/get_all_cpc_ads_daily_performance",{shopId:shop.shop_id,accessToken:shop.access_token,params:{start_date:startDate,end_date:endDate}});}
