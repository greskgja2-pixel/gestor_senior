...
async function writeState(state){state.updatedAt=now();state.lastHeartbeat=now();state.percent=calcPercent(state);await chrome.storage.local.set({[STATE_KEY]:state});return state;}
...
async function scan(tabId){
  const [{result}]=await chrome.scripting.executeScript({target:{tabId},world:'ISOLATED',func:()=>{
    const danger=/(excluir|apagar|deletar|remover|cancelar\s+pedido|cancelar\s+venda|comprar|pagar|pagamento|publicar|confirmar|finalizar|salvar\s+na\s+shopee|desconectar|sair\s+da\s+conta|logout|alterar\s+senha|trocar\s+senha|encerrar\s+conta|banir|suspender)/i;
    ...
    for(const el of nodes){if(elements.length>=160||!visible(el))continue;const text=clean(el.innerText||el.value||el.getAttribute('aria-label')||el.title);const href=el.href||null;const sel=selector(el);if(!sel||seen.has(sel))continue;seen.add(sel);const dangerous=danger.test(`${text} ${href||''}`);elements.push({selector:sel,text,tag:el.tagName.toLowerCase(),role:el.getAttribute('role')||null,href,dangerous,rect:(()=>{const r=el.getBoundingClientRect();return{x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)}})()});}
...
async function processTask(state,task){
  ...
  if(task.dangerous){state.skipped++;state.logs.push({at:now(),type:'skip-danger',task});return;}
...
chrome.runtime.onMessage.addListener((message,_sender,sendResponse)=>{
  ...
  (async()=>{
    try{
      ...
      if(message.type==='GS_MAPPER_STATUS')return{ok:true,state};
      if(message.type==='GS_MAPPER_PAUSE'&&state){state.status='paused';return{ok:true,state:await writeState(state)};}
      if(message.type==='GS_MAPPER_RESUME'&&state){state.status='running';await writeState(state);runLoop();return{ok:true,state};}
