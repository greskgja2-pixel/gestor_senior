...
export function parseIds(url=location.href){const s=String(url);let m=s.match(/-i\.(\d+)\.(\d+)/);if(m)return{shopId:m[1],itemId:m[2]};m=s.match(/[?&]shopid=(\d+).*?[?&]itemid=(\d+)/i);if(m)return{shopId:m[1],itemId:m[2]};return{shopId:null,itemId:null};}
...
export async function collectSearchResults({max=20,scrollSteps=8,delayMs=650}={}){
  ...
  for(let step=0;step<scrollSteps;step++){
    for(const a of document.querySelectorAll('a[href*="-i."]')){try{const row=cardFromLink(a);if(row.itemId&&row.title.length>=4&&!seen.has(row.itemId))seen.set(row.itemId,row);}catch{}}
