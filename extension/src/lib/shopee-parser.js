...
export function parseCurrentProduct(){
  ...
  const ratingMatch=body.match(/(?:^|\s)([0-5][.,]\d{1,2})\s*(?:de\s*5|estrelas?|avaliação)/i);const rating=numberPt(ratingMatch?.[1]);
  const reviewMatch=body.match(/([\d.,]+\s*(?:mil|k|mi|m)?)\s*(?:avaliações|avaliação|reviews?)/i);const reviewCount=compactPt(reviewMatch?.[1]);
  const soldMatch=body.match(/([\d.,]+\s*(?:mil|k|mi|m)?)\s*vendid[oa]s?/i);const sold=compactPt(soldMatch?.[1]);
