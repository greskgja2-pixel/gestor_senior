(() => {
  ...
  chrome.runtime.onMessage.addListener((msg,_sender,reply)=>{
    ...
    if(msg?.type==='GS_PARSE_CURRENT_PRODUCT'){
      import(chrome.runtime.getURL('src/lib/shopee-parser.js')).then(async m=>{
        const dom=m.parseCurrentProduct();let api=null;try{api=await m.fetchCurrentItemApi();}catch{}
        const pick=(a,b)=>a===null||a===undefined||a===''?b:a;
        reply({ok:true,product:{...dom,rating:pick(api?.rating,dom.rating),reviewCount:pick(api?.reviewCount,dom.reviewCount),sold:pick(api?.sold,dom.sold),stock:pick(api?.stock,dom.stock),price:pick(dom.price,api?.price),priceBeforeDiscount:pick(api?.priceBeforeDiscount,dom.priceBeforeDiscount)}});
      }).catch(e=>reply({ok:false,error:String(e)}));return true;
    }
