...
export default async function ProdutosPage() {
  ...
  let items = [], source = null, syncedAt = null, loadError = null;
  try {
  ...
  return (
    <div className="card">
      <div className="meta-row">
        ...
        <RefreshButton apiPath="/api/shopee/products" />
      </div>
      <div className="sub">
      ...
      {!loadError && items.length === 0 && (
        <div className="empty-note">Nenhum produto encontrado (ou a sincronização ainda não rodou — clique em Atualizar).</div>
      )}
      ...
      {items.length > 0 && (
        <table>
          ...
          <tbody>
            {items.map((it) => {
              const price = it.price_info?.[0]?.current_price;
              const stock = it.stock_info_v2?.summary_info?.total_available_stock;
              return (
                <tr key={it.item_id}>
                  <td><b>{it.item_name}</b><br /><span style={{ fontSize: 11, color: "var(--text-muted)" }}>ID {it.item_id}</span></td>
