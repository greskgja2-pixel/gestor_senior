...
function safeText(value, max = 500) { if (value === null || value === undefined) return null; return String(value).trim().slice(0, max) || null; }
async function itemBelongsToShop(db, shopId, itemId) { const { data, error } = await db.from("products_cache").select("item_id").eq("shop_id", shopId).eq("item_id", itemId).maybeSingle(); if (error) throw new Error(error.message); return Boolean(data?.item_id); }

export async function GET(request) {
  const shop = await getActiveShop();
  if (!shop) return NextResponse.json({ error: "Nenhuma loja autorizada." }, { status: 400 });
  const url = new URL(request.url), itemId = positiveInt(url.searchParams.get("item_id")), limit = Math.min(500, Math.max(1, Number(url.searchParams.get("limit") || 200) || 200)), db = supabaseAdmin();
  let query = db.from("extension_analysis_reports").select("id,item_id,analyzed_at,next_reanalysis_at,extension_version,source,objective,situation,bottleneck,score,product_snapshot,ads_snapshot,finance_snapshot,competitors,report,suggestions,metrics,created_at").eq("shop_id", shop.shop_id).order("analyzed_at", { ascending: false }).limit(limit);
  if (itemId) query = query.eq("item_id", itemId);
  const [{ data: reports, error }, { data: schedules, error: scheduleError }] = await Promise.all([query, db.from("extension_analysis_schedules").select("id,item_id,title,product_url,enabled,frequency_days,mode,next_run_at,last_run_at,last_report_id,settings,created_at,updated_at").eq("shop_id", shop.shop_id).order("updated_at", { ascending: false })]);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  if (scheduleError) return NextResponse.json({ error: scheduleError.message }, { status: 500 });
  return NextResponse.json({ reports: reports || [], schedules: schedules || [], shop: { shopId: shop.shop_id, shopName: shop.shop_name || null } });
}
