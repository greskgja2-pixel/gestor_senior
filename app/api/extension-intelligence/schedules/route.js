...
export async function POST(request) {
  ...
  let nextRunAt = body?.next_run_at ? new Date(body.next_run_at) : new Date(Date.now() + frequencyDays * 86400000);
  if (Number.isNaN(nextRunAt.getTime())) return NextResponse.json({ error: "next_run_at inválido." }, { status: 400 });

  const db = supabaseAdmin();
  const { data: exists, error: productError } = await db
    .from("products_cache")
    .select("item_id")
    .eq("shop_id", shop.shop_id)
    .eq("item_id", itemId)
  ...
  const { data, error } = await db.from("extension_analysis_schedules").upsert(
    {
      shop_id: shop.shop_id,
      item_id: itemId,
      title: text(body?.title, 300),
      product_url: text(body?.product_url ?? body?.url, 1000),
      enabled,
      frequency_days: frequencyDays,
      mode,
      next_run_at: enabled ? nextRunAt.toISOString() : null,
      settings,
      updated_at: new Date().toISOString(),
    },
    { onConflict: "shop_id,item_id" }
  ).select("id,item_id,title,product_url,enabled,frequency_days,mode,next_run_at,last_run_at,last_report_id,settings,updated_at").single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
