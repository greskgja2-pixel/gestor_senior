...
const withDefaults=value=>({...defaultCategories,...(value&&typeof value==='object'?value:{})});
...
export async function GET(){
  ...
  const preferences=data?{...data,categories:withDefaults(data.categories)}:{
    ...
    timezone:'America/Sao_Paulo',ads_zero_sales_spend_threshold:10,categories:withDefaults(null)
  };
  return NextResponse.json({ok:true,preferences,providers:getNotificationProviderStatus()});
}
...
export async function POST(request){
  ...
  const email=text(body?.email,320),phone=text(body?.phone,40);
  if(email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return NextResponse.json({error:'E-mail inválido.'},{status:400});
  const phoneDigits=String(phone||'').replace(/\D/g,'');
  if(phone&&phoneDigits.length<10)return NextResponse.json({error:'Celular / WhatsApp inválido.'},{status:400});
  const row={
    shop_id:shop.shop_id,display_name:text(body?.display_name,120),email,phone,
    ...
    categories:withDefaults(body?.categories),
    updated_at:new Date().toISOString()
  };
  const {data,error}=await supabaseAdmin().from('gs_notification_preferences').upsert(row,{onConflict:'shop_id'}).select('*').single();
  if(error)return NextResponse.json({error:error.message},{status:500});
  return NextResponse.json({ok:true,preferences:{...data,categories:withDefaults(data?.categories)},providers:getNotificationProviderStatus()});
}
