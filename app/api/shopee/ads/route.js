...
const METRIC_KEYS=new Set(["impression","impressions","click","clicks","expense","cost","spend","broad_gmv","gmv","broad_order","order","orders","broad_order_amount","sold","broad_roi","broad_roas","roas","roi"]);
...
function settingsMap(result){const map=new Map();for(const row of campaignNodes(result?.data?.response||result?.data||result,[])){const id=Number(row.campaign_id);if(!id)continue;map.set(id,{campaignId:id,itemId:Number(first(row,["item_id","itemid"]))||null,title:String(first(row,["campaign_name","name","title"])||`Campanha ${id}`),state:first(row,["status","state","campaign_status"])??null,budget:num(first(row,["daily_budget","budget"]))||null});}return map;}
...
export async function GET(request){
  ...
  // e desempenho diário por campanha no período pedido.
  ...
  const summary=aggregateRows(dailyRows);
  ...
  return NextResponse.json({days,shopId:String(shop.shop_id),campaignIds:ids,daily,hourly,campaigns,settings,campaignDaily,v5:{source:"Enciclopédia Shopee Seller Center v5",summary,campaigns:normalizedCampaigns,insights}});
}
