...
export async function POST(request){
  ...
  let visualImages=0;
  visualImages+=await appendImageGroup(parts,'IMAGENS DO ANÚNCIO DO USUÁRIO — compare estas imagens com os concorrentes selecionados.',safeArray(p.imageUrls||p.image?.image_url_list));
  for(let i=0;i<comps.length;i++){
    const c=comps[i]||{};
    const urls=safeArray(c.imageUrls||c.images||c.image?.image_url_list);
    visualImages+=await appendImageGroup(parts,`IMAGENS DO CONCORRENTE ${i+1} — título: ${String(c.title||'não informado')} — vendidos: ${String(c.sold??'não informado')} — preço: ${String(c.price??'não informado')}`,urls.slice(0,2));
  }
  ...
  const newReport={...safeObject(row.report),ai_analysis:analysis,ai_model:model,ai_analyzed_at:new Date().toISOString(),visual_images_sent:visualImages};
  ...
  return NextResponse.json({ok:true,analysis,model,visualImages});
}