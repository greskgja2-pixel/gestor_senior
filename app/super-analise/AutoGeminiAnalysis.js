...
export default function AutoGeminiAnalysis({reportId,itemId}){
  ...
  async function run(){
    ...
    try{
      ...
      setProgress(100);
      setMessage(kind==='timeout'?'A análise de I.A. excedeu 2 minutos. Tente novamente.':'A análise de I.A. não pôde ser concluída agora. Tente novamente.');
    }
  }
  ...
  return <div className="gs-ai-progress-overlay" role="status" aria-live="polite">
    <div className="gs-theme-floating-card gs-ai-progress-card" data-state={state}>
      <div className="gs-ai-progress-head">
        <b className={state==='error'?'gs-theme-error':''}>{state==='error'||state==='timeout'?'⚠ A análise de I.A. não concluiu':state==='done'?'✓ I.A. concluiu':'✦ Analisando pela I.A.…'}</b>
