...
export default function WebAuditFlow({initialUrl=''}){
  ...
  async function saveAndContinue(){
    ...
    try{
      ...
      setStep(3);setMessage('Agora escolha exatamente 3 concorrentes. O Motor Senior abrirá a busca normal da Shopee.');
    }catch(e){setMessage(String(e?.message||e));}finally{setLoading(false);}
  }

  async function openPicker(){
    setMessage('Abrindo a busca da Shopee. Se “Selecionar” não aparecer, use Recarregar botões.');
    try{
      ...
      pollRef.current=setInterval(async()=>{try{const r=await engine('pickerResult',{requestId:req.requestId},8000);if(!r?.done)return;clearInterval(pollRef.current);setPicker(null);if(r.cancelled)return setMessage('Seleção cancelada. Você pode abrir a busca novamente.');const items=(r.items||[]).slice(0,3);setCompetitors(items);if(items.length===3){setStep(4);setMessage('3 concorrentes recebidos. Revise o resumo e clique em Analisar Tudo.');}else setMessage(`${items.length} concorrente(s) recebidos. Selecione exatamente 3.`);}catch{}},1100);
  ...
  async function analyzeAll(){
    ...
    setAnalyzing(true);setMessage('Motor Senior coletando descrição, imagens, vídeo, atributos, variações, categoria e oferta dos 3 concorrentes…');
    try{
      ...
      setMessage('Coleta concluída. Abrindo Original × Sugestão da IA no Gestor…');
  ...
  return <div className={styles.screen}>
    ...
    <main className={styles.main}>
      ...
      {step===3&&<section className={styles.card}><div className={styles.cardHead}><div><span className={styles.num}>3</span><h2>Selecione exatamente 3 concorrentes</h2></div><Help>O Motor Senior abre uma busca normal da Shopee e adiciona o botão “Selecionar” nos anúncios.</Help></div><p>Escolha anúncios realmente comparáveis. Depois de selecionar os três, volte ao Gestor.</p><div className={styles.actionsLeft}><button className={styles.primary} onClick={openPicker}>{picker?'Busca aberta':'Abrir busca da Shopee'}</button><button onClick={reloadPicker}>↻ Recarregar botões</button></div>{competitors.length>0&&<div className={styles.competitors}>{competitors.map((c,i)=><article key={`${c.itemId||i}`}><>{c.imageUrl?<img src={c.imageUrl} alt=""/>:<div className={styles.noImg}/>}</><div><b>{i+1}. {c.title||'Concorrente'}</b><small>{money(c.price)} · {c.sold??'—'} vendidos · {c.rating??'—'} ★</small></div></article>)}</div>}<div className={styles.actions}><button onClick={()=>setStep(2)}>‹ Voltar</button>{competitors.length===3&&<button className={styles.primary} onClick={()=>setStep(4)}>Revisar e analisar ›</button>}</div></section>}

      {step===4&&<><section className={styles.card}><div className={styles.cardHead}><div><span className={styles.num}>4</span><h2>Revisão antes de Analisar Tudo</h2></div><Help>Nada é alterado na Shopee nesta etapa. O Motor Senior apenas coleta o pacote completo para o Gemini.</Help></div><div className={styles.reviewGrid}><article><small>Objetivo</small><b>{objective}</b><span>{situation} · gargalo: {bottleneck}</span></article><article><small>Ads</small><b>ROAS {ads.roas||'—'} · alvo {ads.targetRoas||'—'}</b><span>GMV {money(ads.gmv)} · gasto {money(ads.spend)} · custo/venda {money(ads.costPerSale)}</span></article><article><small>Custos</small><b>{models.length?`${variationCosts.filter(x=>n(x.cost)!=null).length}/${models.length} variações com custo`:money(baseCost)}</b><span>Margens serão calculadas com preço e custo correspondentes.</span></article><article><small>Concorrentes</small><b>{competitors.length}/3 selecionados</b><span>Serão coletados título, descrição, imagens, vídeo, preço, avaliações, categoria, atributos e variações.</span></article></div><div className={styles.finalBox}><b>O que acontece agora?</b><p>O Motor Senior coleta profundamente os 3 concorrentes e envia anúncio + contexto + Ads + custos + margens + variações + imagens ao Gestor. Depois você verá Original × Sugestão da IA, justificativas SEO/AIDA/concorrentes, análise visual das imagens, categoria oficial Shopee e notas Antes × Depois.</p></div><div className={styles.actions}><button onClick={()=>setStep(3)}>‹ Voltar aos concorrentes</button><button className={styles.primaryBig} disabled={analyzing||competitors.length!==3} onClick={analyzeAll}>{analyzing?'Coletando e preparando…':'✦ Analisar Tudo'}</button></div></section></>}
    </main>
  </div>;
}