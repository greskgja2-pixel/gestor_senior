...
export default function WebAuditFlow({initialUrl=''}){
  ...
  const variationMargins=useMemo(()=>models.map(m=>{const row=variationCosts.find(x=>String(x.modelId)===String(m.modelId));return{modelId:m.modelId,name:m.name||row?.name||'Variação',price:n(m.price),cost:n(row?.cost),margin:estimatedMargin(m.price,row?.cost,ads.costPerSale)}}),[models,variationCosts,ads.costPerSale]);
