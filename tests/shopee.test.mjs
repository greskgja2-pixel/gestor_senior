...
test('authorization, token exchange, refresh and shop signing', async (t) => {
  ...
  process.env.SHOPEE_PARTNER_ID = ' 1240696 ';
  ...
  assert.equal(auth.origin, 'https://partner.test-stable.shopeemobile.com');
  assert.equal(auth.searchParams.get('partner_id'), '1240696');
  assert.equal(auth.searchParams.get('sign'), hmac('1240696/api/v2/shop/auth_partner1700000000'));
  let calls = [];
  ...
  assert.equal(calls[0].url.searchParams.get('sign'), hmac('1240696/api/v2/auth/token/get1700000000'));
  assert.deepEqual(JSON.parse(calls[0].options.body), { code: 'code', shop_id: 123, partner_id: 1240696 });
  assert.equal(calls[0].options.cache, 'no-store');
  await api.refreshAccessToken('refresh', 123);
  assert.equal(calls[1].url.searchParams.get('sign'), hmac('1240696/api/v2/auth/access_token/get1700000000'));
  await api.getItemList({ shopId: 123, accessToken: 'access' });
  assert.equal(calls[2].url.searchParams.get('sign'), hmac('1240696/api/v2/product/get_item_list1700000000access123'));
